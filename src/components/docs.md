...
- **App.vue** -- Root component. Owns the `state` reactive object (`currentRound`, `completedRounds`, `inputLetters`, timing state, `transitioning` flag), a `hintIndex` ref tracking which offered letter is highlighted as a hint (or `null`), and an `isFreshGame` ref distinguishing first-time completion from saved game reload. Loads puzzle data on mount, checks localStorage for saved games (with backfill of `offeredLetters` for old saves), runs the round timer, and handles submit/skip/hint/advance lifecycle. Emits nothing; receives events from children. Supports two input paths: keyboard input (physical + virtual) via `handleKeyInput`, and click-to-place/remove via the same `handleKeyInput` for source tile clicks and `handleInputTileClick` for removing specific input letters by index.
