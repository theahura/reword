...
- `filterByCommonWords` drops any root whose expansion words contain zero words from the 50K common word set. This is a root-level filter -- if at least one expansion word across all letter-groups is common, the entire root and all its expansions (including obscure words) are kept.
- `trimPuzzleData` caps roots at 500 per length (preferring roots with more single-letter expansions) and caps words per expansion key at 5.
...
- The output JSON structure is keyed by root length (string keys like `"3"`, `"4"`, etc.), with each value being an array of `{ root, expansions }` objects where `expansions` maps added-letter strings to arrays of valid words.
