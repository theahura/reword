...
- **Time penalty for wrong answers**: A `'wrong'` feedback (real word that is not a valid expansion) deducts time from `state.roundDeadline` and shows a brief flash on `.timer-display` plus a floating `-5s` badge in the timer slot. This is the only place outside `startTimer()` and the round-end skip handler that mutates `roundDeadline` — anything reading the timer must treat the deadline as user-input-mutable mid-round. The penalty is a no-op when `timerDisabled` is true (untimed mode), so timed and untimed modes diverge here. The penalty does NOT fire for `'profanity'`, `'invalid-length'`, or `'trivial-suffix'` (trivial inflections are treated as honest mistakes). The penalty does not affect `state.completedRounds[].timeMs`, which is computed from `roundStartTime` at round completion. The flash and badge timeouts are tracked at module scope and cleared in `onUnmounted` to prevent leaks across hot reloads. Both animations are suppressed under `prefers-reduced-motion` via rules in `@/style.css`.
