...
- **App.vue** -- Root component. Owns the `state` reactive object (`currentRound`, `completedRounds`, `inputLetters`, timing state, `transitioning` flag), a `hintIndex` ref tracking which offered letter is highlighted as a hint (or `null`), an `isFreshGame` ref distinguishing first-time completion from saved game reload, a `solveRates` ref (Array or null) for per-round solve percentages, and an `answerCounts` ref (Array of per-round `{word: count}` maps, or null) for per-answer popularity in the modal. Loads puzzle data on mount, checks localStorage for saved games (with backfill of `offeredLetters` for old saves), runs the round timer, and handles submit/skip/hint/advance lifecycle. At game end (`showScore()`), submits results to Firestore fire-and-forget for fresh games and always fetches solve rates for display. The fetch returns a single `{ rates, answerCounts }` object (or `null` when fewer than 10 players have completed the puzzle); `App.vue` destructures it into the two refs together, so they are populated atomically. Emits nothing; receives events from children. Supports two input paths: keyboard input (physical + virtual) via `handleKeyInput`, and click-to-place/remove via the same `handleKeyInput` for source tile clicks and `handleInputTileClick` for removing specific input letters by index.
...
- **ScoreScreen.vue** -- Receives `results` (the `completedRounds` array), `isFreshGame`, and `solveRates` (Array or null) as props. Displays stats, countdown to next puzzle, a share button, and a per-round summary rendered as a semantic HTML `<table>` (with column headers "Root", "Result", and conditionally "Solved by") inside a scrollable `.rounds-summary-wrap` div. The heading (e.g., "Congrats!") is rendered using `TileText` with the `animate` prop, so letters pop in with staggered delays to mimic the gameplay typing feel. The share button is a standard styled button (green background, white text, rounded corners) with plain text -- it does not use `TileText` or tile components. When `solveRates` is provided, each round row displays a percentage showing what fraction of players solved that round (Sporcle-style social comparison). On mount, if `isFreshGame` is true and `isAllSolved(results)` passes, fires a `canvas-confetti` burst (with `disableForReducedMotion: true` for accessibility). This ensures confetti only fires on first completion, not on saved game reload.

- **WordListModal.vue** -- Overlay modal opened when the user clicks "+N more" on any round in `ScoreScreen`. Receives a `round` object (from `completedRounds`), `roundIndex`, and an optional `answerCounts` prop (a `{word: count}` map for that round, or `null`). The modal has a fixed header (close button + h2 title) and a scrollable `.modal-body` div that wraps the three content sections (root word tiles, offered letter tiles, and possible word rows). The body scrolls independently via `overflow-y: auto` with `max-height: calc(80vh - 120px)`, keeping the header pinned. Uses `matchTypedToTiles()` from `@/src/game.js` to determine which letters in each word came from root vs offered pool, applying green styling to offered-source tiles. When `answerCounts` is provided, a `displayedAnswers` computed sorts `round.possibleAnswers` by submission count descending and renders an `X%` label (`.word-popularity`) next to each row, computed as `count / sum(counts)`. The row whose word matches `round.answer` (case-sensitive equality with the saved answer) gets the `.player-answer` class, which colors that row's percentage label amber. When `answerCounts` is `null` (data unavailable or `<10 games` gate), words render in their original `possibleAnswers` order with no percentage labels.

- **TileText.vue** -- Reusable component that renders a text string as letter tiles, matching the game's tile aesthetic. Takes `text`, optional `tileClass`, and optional `animate` (boolean) props. Splits the text on spaces into words, rendering each word as a row of `ScrabbleTile` components. When `animate` is true, each tile gets the `.tile-animate` CSS class and a staggered `animationDelay` (0.07s per letter, computed globally across words), producing a sequential pop-in effect (`scale(0.5)` to `scale(1)`, 0.15s). The animation respects `prefers-reduced-motion`. Used by `ScoreScreen` for the heading. Styling in `@/style.css` sizes tiles at 32px (24px on narrow viewports) -- smaller than gameplay tiles.
...
- **`solveRates` and `answerCounts` data flow**: `App.vue` owns `solveRates` (ref, Array of 10 percentages or null) and `answerCounts` (ref, Array of 10 `{word: count}` maps or null), both default `null`. In `showScore()`, for fresh games, `submitGameResults()` is called fire-and-forget (errors silently caught). For both fresh and saved games, `fetchSolveRates()` is awaited; on success it returns `{ rates, answerCounts }`, which is destructured into both refs in a single step. The fetch returns `null` (and the refs stay `null`) when the daily document has fewer than 10 submissions or does not exist, so the `<10 games` gate suppresses both UIs in lockstep. `solveRates` is passed to `ScoreScreen` for the per-round solve percentage column; the per-round slice `answerCounts[wordListRoundIndex]` is passed to `WordListModal` for the popularity ordering and percentages. Since fetch is async and can fail, the UI gracefully omits both displays when the refs are null. See `@/src/leaderboard.js` for the Firestore shape and `@/firestore.rules` for the field whitelist.
