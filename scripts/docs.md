...
- `trimPuzzleData` performs a shallow copy of the puzzle data, preserving `root`, `expansions`, `trivialAnswers`, `commonKeys`, and `commonWords` from each entry. It caps roots per length bucket to `MAX_ROOTS_PER_LENGTH` (500). When a bucket exceeds the cap, roots are sorted by quality (preferring roots with more single-letter expansion keys, then more total expansion keys) and the top entries are kept. All words within each surviving root's expansions are preserved without truncation.
...
- **Root count cap**: `trimPuzzleData` caps roots per length bucket to 500, prioritizing roots with the most single-letter expansion keys. All words within each surviving root's expansions are preserved without truncation. The cap applies to root count only, not to expansion word lists.
